...
static simple_ble_char_t led_light = {.uuid16 = 0xaeef};
...
void ble_evt_write(ble_evt_t const* p_ble_evt) {
    if (simple_ble_is_char_event(p_ble_evt, &led_light)) {
      printf("Got write to LED characteristic!\n");
      //display_write((char*)light, DISPLAY_LINE_0);
    } 
}
...
int main(void) {
  ...
  while(1) {
    if (read_sensor) {
      light = max44009_read_lux();
      simple_ble_notify_char(&led_light);
      read_sensor = false;
      printf("%f", light);
    }
